...
between the migrations and the models. Views are created and dropped
explicitly: a `create_all` test database needs them created separately, and a
downgrade drops views before the tables they read.
...
creates their table, as is the own-store `sales_venue` row.
